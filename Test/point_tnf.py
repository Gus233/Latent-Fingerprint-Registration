...
class PointTnf(object):
  ...
  #      self.tpsTnf = TpsGridGen(use_cuda=self.use_cuda)
    ...
    def affPointTnf(self,theta,points):
        ...
        theta_mat = theta.view(-1, 1, 3)
        warped_points =  points + theta[:, 0:2].unsqueeze(2).expand_as(points)
        theta_mat = torch.cat((torch.cos(theta[:,2]).unsqueeze(1), -torch.sin(theta[:,2]).unsqueeze(1), torch.sin(theta[:,2]).unsqueeze(1), torch.cos(theta[:,2]).unsqueeze(1)),1)
        theta_mat = theta_mat.view(-1, 2, 2)
